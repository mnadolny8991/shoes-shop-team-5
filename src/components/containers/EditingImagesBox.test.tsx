...
describe('EditingImagesBox', () => {
    test('render', () =>{
        render(<EditingImagesBox error="test error" initialImages={mockProduct.images} onChange={()=>{}}/>)
    ...
    test('upload image', async() => {
        ...
        expect(onChange).toHaveBeenCalledWith({images:[], uploadedImages:[jpegFile]})
    ...
    test('upload multiple images', async() => {
        const onChange = jest.fn()
        render(<EditingImagesBox onChange={onChange} initialImages={mockProduct.images}/>)
        ...
        await waitFor(() => expect(onChange).toHaveBeenCalled())
        expect(onChange).toHaveBeenCalledWith({images: initialIds, uploadedImages:[jpegFile, pngFile]})
    ...
    test('delete dialog', async () => {
        ...
        const deleteModalTitle = screen.queryByText("Are you sure to delete product image")
    ...
    test('delete image', async () => {
        ...
        await waitFor(() => expect(screen.queryByText("Are you sure to delete product image")).not.toBeInTheDocument())

        expect(onChange).toHaveBeenCalledWith({images:[initialIds[0]], "uploadedImages": []})
    ...
    test('delete uploaded image', async () => {
        ...
        expect(onChange).toHaveBeenCalledWith({images:initialIds, "uploadedImages": [pngFile]})
        expect(imageBox).not.toBeInTheDocument()
    });
});