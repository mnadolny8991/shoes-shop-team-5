import { screen, fireEvent, getAllByRole, render, act, getByAltText} from '@testing-library/react';
...
jest.mock('../../hooks/categories' , () => ({
    ...
    useGenders:() => ({data: [{id:3, name:"Men"}, {id:4,name:"Women"}], isLoading: false})
}))
...
describe("no initial data", ()=> {
    beforeEach(async () => {
        await act(async () => {
            render(<ProductForm title="product title" description="Provide detailed information" onSubmit={onSubmit}/>)
        })
    });
    ...
    test('upload and delete image file', async() => {
        ...
        await act(async() => fireEvent.click(screen.getByTestId("delete-btn-modal")))
...
describe('based on existing product', () => {
    beforeEach(() => {
        render(<ProductForm title="product title" description="Provide detailed information" onSubmit={onSubmit} product={mockProduct} submitDirty/>)
    ...
    test("render initial product", () => {
        ...
        allSizes.forEach(({id, name})=>{
            const sizeCheckbox = screen.getByLabelText(name)
            mockSelectedSizesIds.includes(id) ? expect(sizeCheckbox).toBeChecked() : expect(sizeCheckbox).not.toBeChecked()
        })
    ...
    test('submit only dirty data: color, gender, description, upload images', async () => {
        ...
        const colors : HTMLSelectElement = screen.getByLabelText("Color")
        fireEvent.change(colors, {target:{value:changedProductProps.color}})

        const gender = screen.getByLabelText("Gender")
        fireEvent.change(gender, {target:{value:changedProductProps.gender}})

        const description = screen.getByLabelText("Description")
        fireEvent.change(description, {target:{value: changedProductProps.description}})
