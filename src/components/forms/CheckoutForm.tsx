...
import { addressValidator, emailValidator, nameValidator, phoneValidator, zipCodeValidator } from "@/lib/validators";
...
export default function CheckoutForm() {
  ...
  const { data: session } = useSession();
    const [confirmPaymentErrorMessage, setConfirmPaymentErrorMessage] = useState<string | null>(null);
    const [isLoading, setIsLoading] = useState(false);
    const [firstName, setFirstName] = useState('');
    const [lastName, setLastName] = useState('');
    const [email, setEmail] = useState('');
    const [phoneNumber, setPhoneNumber] = useState('');
    const [country, setCountry] = useState<CountryType | null>(countries.find(country => country.code === "US") ?? null);
    ...
    const [isFirstInteractionCountry, setIsFirstInteractionCountry] = useState(false);
    ...
    const [isFirstInteractionZipCode, setIsFirstInteractionZipCode] = useState(false);
    const [isFirstInteractionAddress, setIsFirstInteractionAddress] = useState(false);
    ...
    const countryError  = isFirstInteractionCountry && !country ? "Please specify your country" : ""
  ...
  const { data, status } = session ? useUserData(session.id, session.accessToken) : {};
  ...
  useEffect(() => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if(!firstName)
          setIsFirstInteractionFirstName(true);
        if(!lastName)
          setIsFirstInteractionLastName(true);
        if(!email)
          setIsFirstInteractionEmail(true);
        if(!phoneNumber)
          setIsFirstInteractionPhone(true);
        if(!country)
          setIsFirstInteractionCountry(true);
        if(!state)
          setIsFirstInteractionState(true);
        if(!city)
          setIsFirstInteractionCity(true);
        if(!zipCode)
          setIsFirstInteractionZipCode(true);
        if(!address)
          setIsFirstInteractionAddress(true);
    ...
    return(
        <form onSubmit={handleSubmit} id="checkoutForm">
          <Typography variant="h1" mb={{xs:5, sm:7, md:9}}>Checkout</Typography>
          <Typography variant="h4" mb={{xs:3, md:4}}>Personal info</Typography>
          <Stack spacing={{xs:2,sm:3}} direction={'row'} mb={{xs:2, sm:3, md:4}}>
            <TextField
              value={firstName}
          ...
          <Typography variant="h4" mb={{xs:3, md:4}}>Shipping info</Typography>
          <Grid2 container spacing={{xs:2,sm:3}} mb={{xs:1, sm:2, md:3}}>
            <Grid2 xs={6} md={3}>
              <CountryAutocomplete
                value={country}
                onBlur={(e) => {setIsFirstInteractionCountry(true); console.log("country blur")}}
                onChange={setCountry}
                error={countryError}
              />
            </Grid2>
            ...
            <Divider sx={{my:{xs:5, sm:7, md:9}}}/>
            <Typography variant="h4" mb={{xs:3, md:4}}>Payment info</Typography>
