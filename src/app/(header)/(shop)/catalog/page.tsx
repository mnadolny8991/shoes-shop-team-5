...
export default async function Page({ searchParams }: {
  ...
  const filters: Filters = {
    ...
    price: (JSON.parse(searchParams!['price'] as string ?? 'null')) as [number, number] ?? [0, 999],
  ...
  await queryClient.prefetchQuery({
    ...
    queryFn: () => fetchProductsByFiltersAndName(filters, searchText, page, pageSize, '*', 'updatedAt', 'desc', { cache: 'no-store' }),
  });
