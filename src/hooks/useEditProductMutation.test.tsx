...
describe('edit product mutation hook', () => {
  ...
  test('edit product successfull', async () => {
    ...
    (fetchFilesByIds as jest.Mock).mockImplementation(
      (imagesToDelete: number[]) => imagesToDelete.map((id) => ({ id }))
    );
    ...
    const { result } = renderHook(() => useEditProductMutation(1, 'token'), {
      wrapper,
    });
  ...
  test('upload images and then edit product successfull', async () => {
    ...
    expect(updateProduct).toHaveBeenCalledWith(
      { name: 'Nike Kyrie 998', images: [1, 2, 3, 4] },
      1,
      'token'
    );
    expect(result.current.errorEditingProduct).toBeFalsy();
